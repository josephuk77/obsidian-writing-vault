...
### Docker local ddl-auto 보강 후 기동 성공

- 문제: credential 정리 후에도 local Docker DB가 비어 있으면 `ddl-auto=validate` 상태에서 `users` 테이블 부재로 앱이 기동하지 못했다.
- 결정: 로컬 Docker 개발 검증용으로만 `SPRING_JPA_HIBERNATE_DDL_AUTO=update`를 기본값으로 사용한다.
- 해결: `application-local.yml`과 `docker-compose.yml`에 local ddl-auto 기본값 `update`를 설정했다.
- 검증: `docker compose build app`, `docker compose up -d app`, `GET /api/v1/health` 모두 성공했고 Hibernate가 `users` 테이블을 생성했다.
- 남은 범위: 운영 migration 전략과 최종 Flyway migration consolidation은 후속 작업으로 유지한다.
