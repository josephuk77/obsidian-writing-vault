...
- 기준 문서 확인: Vault `AGENTS.md`, repo `AGENTS.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`, Obsidian `Projects/FaithLog/resume-metrics.md`, `Projects/FaithLog/decision-log.md`를 확인하고 진행했다. 저장소에는 이번 실행도 `AGENT.md`가 없고 `AGENTS.md` 단일 규칙만 유지 중이다.
- 보고 기준일은 일요일 `2026-07-26`이다. 로컬 `date` 출력은 `2026-07-27 06:01:16 +0900`이므로 future-dated local clock evidence로만 기록한다.
- 지난 자동화 실행 시각(`2026-07-25T21:07:58.749Z`) 이후 현재 체크아웃 브랜치 새 커밋 0건, 같은 기간 `origin/develop` 비-merge 실변경 커밋 0건. 최신 upstream 실변경 커밋은 여전히 `c7d67b4 fix: #213 투표 조회에 항목 추가 허용값 노출 (#214)` (`2026-07-20 16:21:33 +0900`, 6 files changed / +28 / -2)다.
- 현재 브랜치 상태: `docs/37-poll-template-planning-sync`, divergence `ahead 4 / behind 213`, branch 고유 diff `3 files changed, 104 insertions(+)`, 미커밋 diff `4 files changed, 2657 insertions(+), 9 deletions(-)`, 미추적 빈 파일 `0` 유지.
- 코드베이스 구조 수치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 main Java sources, 28 test Java sources, 55 snippet groups, 2 GitHub Actions workflows, checked-out branch DB 마이그레이션 0개. 로컬 `origin/develop` snapshot은 575 main Java sources, 102 test Java sources, DB 마이그레이션 12개다.
- `./gradlew test --warning-mode all --console=plain`: 실패, 53초, `5 actionable tasks: 1 executed, 4 up-to-date`. `build/test-results/test/*.xml` 집계는 24 XML files, 138 tests / 2 failures / 0 errors / 0 skipped로 통과율 98.55%다. 실패 테스트는 `BillingApiRestDocsTest.documents_charge_query_contracts`, `BillingControllerTest.charge_query_api_maps_my_summary_admin_campus_and_admin_member_responses`다.
- `./gradlew build --warning-mode all --console=plain`: 실패, 52초, `8 actionable tasks: 1 executed, 7 up-to-date`. 동일 2개 Billing 실패 외에 `TEST-com.faithlog.user.presentation.UserMeControllerTest.xml`, `TEST-com.faithlog.user.presentation.AuthRefreshControllerTest.xml` 2건에 대해 "Could not write XML test results" 오류가 추가 출력됐다.
- build artifact 시각: `build/reports/tests/test/index.html`, `build/reports/problems/problems-report.html` 모두 `2026-07-27 06:02:46 +0900`로 갱신됐다. monitor date `2026-07-26`보다 하루 앞선 future-dated local timestamp다.
- 운영/배포 신호: `docker ps`에서는 `faithlog-latest-app` Up 6 days, `faithlog-latest-redis` / `faithlog-latest-postgres` Up 6 days (`healthy`)가 보였다. 호스트 경로 `http://127.0.0.1:28080/actuator/health`, `http://127.0.0.1:28080/api/v1/health`는 모두 즉시 연결 실패(`http_code=000`)였고, fresh container-internal probe는 `permission denied while trying to connect to the docker API at unix:///Users/josephuk77/.docker/run/docker.sock` 때문에 재수집하지 못했다.
- 오늘 리스크/관찰: Billing summary/REST Docs 회귀 2건 지속, build 게이트 차단 지속, build XML write failure 2건 재발, 브랜치 격차 `ahead 4 / behind 213`, host-path health 실패, local clock future timestamp, 미추적 파일 `0`.
- 오늘 테스트 후보:
  - `./gradlew test --tests com.faithlog.billing.presentation.BillingApiRestDocsTest --tests com.faithlog.billing.presentation.BillingControllerTest`
  - 이유: Billing summary 응답/문서 계약 실패 2건을 좁혀서 재검증해야 한다.
  - 기대 지표: Billing summary 계약 pass/fail, `monthlyByCategory` 필드 존재 여부, `monthlyPaidAmount` 계산값 일치 여부
  - `./gradlew build --rerun-tasks --warning-mode all --console=plain`
  - 이유: XML write failure가 `2026-07-21`의 18건에서 이번 rerun 2건으로 줄었지만 원인 미확인 상태다.
  - 기대 지표: XML write failure 재현 여부, 실패 XML 파일 수, build artifact 기록 안정성
  - `./gradlew test --tests com.faithlog.user.presentation.UserMeControllerTest --tests com.faithlog.user.presentation.AuthRefreshControllerTest`
  - 이유: build 단계 XML write failure가 두 user presentation report 파일에 집중됐다.
  - 기대 지표: 해당 두 테스트 단독 실행 pass/fail, XML result write failure 재현 여부
  - `git switch develop && ./gradlew test --tests com.faithlog.poll.controller.PollApiRestDocsTest`
  - 이유: dirty worktree와 branch-switch 미승인 상태라 최신 upstream poll REST Docs 회귀를 직접 검증하지 못했다.
  - 기대 지표: `allowUserOptionAdd` 노출 계약 pass/fail
- 오늘 트러블슈팅:
  - 문제: Billing summary 관련 테스트 2건이 2026-07-26 rerun에서도 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 `monthlyByCategory` 필드 4개 누락과 `monthlyPaidAmount` 기대값 `2500` 대비 실제 `0` 반환뿐이다.
  - 조치: `./gradlew test`, `./gradlew build`를 fresh rerun해 동일 실패를 다시 수집했다. 구현 변경은 하지 않았다.
  - 전후 지표: 2026-07-21 `138 tests / 2 failures`, 2026-07-26 rerun `138 tests / 2 failures`
  - 예방 후보: 승인된 변경 원천 확인 후 Billing summary DTO/집계/REST Docs 계약 동시 재검증
  - 문제: `./gradlew build`가 `UserMeControllerTest`, `AuthRefreshControllerTest` XML 결과 2건을 쓰지 못했다.
  - 원인: 미확인. `./gradlew test`에서는 24 XML files 집계가 가능했고, 직후 `./gradlew build`에서만 2개 XML write 오류가 보고됐다.
  - 조치: fresh build rerun으로 raw 오류를 다시 수집했다. `clean`이나 파일 삭제 같은 추가 조치는 하지 않았다.
  - 전후 지표: 2026-07-21 build XML write failure 18건, 2026-07-26 rerun build XML write failure 2건
  - 예방 후보: artifact 정리 정책, parallel test write 설정, file lock 조사 범위를 사용자 승인 후 확정
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - resume-ready maintenance candidate: 2026-07-26 rerun 기준 checked-out branch 품질 게이트는 138-test baseline 중 2개 Billing 회귀로 98.55% pass rate에 머물렀고, build path의 XML result write failure는 18건에서 2건으로 줄었지만 승인된 수정이나 원인 확인이 없어 troubleshooting evidence로만 유지한다.

### 2026-07-26 Daily Monitor (checked-out branch)

- 브랜치/작업트리:
  - 기준 문서 확인: repo `AGENTS.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`를 확인하고 진행했다. 저장소에는 이번 실행도 `AGENT.md`가 없고 `AGENTS.md` 단일 규칙만 유지 중이다.
  - 보고 기준일은 일요일 `2026-07-26`이다. 로컬 `date` 출력은 `2026-07-27 06:01:39 +0900`이므로 future-dated local clock evidence로만 기록한다.
  - 현재 브랜치: `docs/37-poll-template-planning-sync`
  - `git status --short --branch`: 워크트리 변경 4건 (`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`), 미추적 빈 파일 1건 (`0`, 0 bytes, mtime `2026-06-30 22:58 +0900`)
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 213
- 변경 범위 수치:
  - 지난 자동화 실행 시각(`2026-07-25T21:07:58.743Z`) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 0건
  - 최신 upstream 실변경 커밋 참고: `c7d67b4 fix: #213 투표 조회에 항목 추가 허용값 노출 (#214)`, `2026-07-20 16:21:33 +0900`, 6 files changed / +28 / -2
  - 최신 upstream 주요 변경 파일: `docs/decision-log.md`, `src/docs/asciidoc/index.adoc`, `src/main/java/com/faithlog/poll/controller/dto/response/PollDetailResponse.java`, `src/main/java/com/faithlog/poll/controller/dto/response/PollListResponse.java`, `src/main/java/com/faithlog/poll/service/result/PollListItemResult.java`, `src/test/java/com/faithlog/poll/controller/PollApiRestDocsTest.java`
  - 현재 브랜치 고유 변경: `git diff --shortstat origin/develop...HEAD` 기준 3 files changed, 104 insertions(+), 모두 문서(`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`)
  - 현재 워크트리 미커밋 변경량: `git diff --shortstat` 기준 4 files changed, 2657 insertions(+), 9 deletions(-)
  - 같은 기간 코드/설정/의존성/마이그레이션 신규 upstream 관찰: 없음
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 main Java sources, 28 test Java sources, 55 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 8 top-level modules, 575 main Java sources, 102 test Java sources, DB 마이그레이션 12개
- 검증 신호:
  - `./gradlew test --warning-mode all --console=plain`: 실패, 51초, `5 actionable tasks: 1 executed, 4 up-to-date`
  ...
  - test 단계 추가 관찰: 같은 `./gradlew test` 실행에서 `build/test-results/test/*.xml` 11건에 대해 "Could not write XML test results" 오류가 함께 출력됐다.
  - deprecated 경고: configure 단계 `StartParameter.isConfigurationCacheRequested` 1건 지속
  - `./gradlew build --warning-mode all --console=plain`: 실패, 35초, `8 actionable tasks: 1 executed, 7 up-to-date`
  - build 단계에서는 동일 2개 Billing 테스트 실패가 재현됐지만, 이번 `build` 실행 본문에는 XML write failure 문구가 다시 출력되지는 않았다.
  - build artifact 시각: `build/reports/tests/test/index.html`, `build/reports/problems/problems-report.html` 모두 `2026-07-27 06:03:26 +0900`로 갱신됐다. monitor date `2026-07-26`보다 하루 앞선 future-dated local timestamp다.
- 운영/배포 신호:
  - `docker ps`: `faithlog-latest-app` Up 6 days, `faithlog-latest-redis` / `faithlog-latest-postgres` Up 6 days (`healthy`)
  - 별도 로컬 컨테이너 `timing-jeju-postgres`도 Up 5 days (`healthy`) 상태로 관찰됐지만 FaithLog 전용 집계에는 포함하지 않았다.
  - 호스트 경로 `curl http://127.0.0.1:28080/actuator/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000104s`
  - 호스트 경로 `curl http://127.0.0.1:28080/api/v1/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000111s`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/actuator/health`: `{"status":"UP"}`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/api/v1/health`: `{"success":true,"code":"SUCCESS","message":"요청이 성공했습니다.","data":{"status":"UP"},"timestamp":"2026-07-26T21:02:48.638558092Z"}`
  - `docker exec faithlog-latest-postgres pg_isready -U faithlog -d faithlog`: `/var/run/postgresql:5432 - accepting connections`
  - `docker exec faithlog-latest-redis redis-cli ping`: `PONG`
- 오늘 리스크/관찰:
  - checked-out branch의 Billing summary/REST Docs 회귀 2건이 2026-07-26 monitor run에서도 그대로 재현됐다.
  - build 게이트는 오늘도 동일 2개 Billing 테스트 실패 때문에 막혀 있다.
  - `./gradlew test`에서 XML test result write failure 11건이 다시 관찰됐지만, 직후 `./gradlew build` 본문에서는 같은 문구가 재출력되지 않아 재현 양상이 일관되지 않다.
  - 브랜치 격차가 `ahead 4 / behind 213`으로 유지돼 현재 브랜치 검증값을 최신 통합선 품질로 인용하기 어렵다.
  - 지난 자동화 실행 이후 신규 upstream 실변경 커밋은 없었다.
  - host-mapped app port `127.0.0.1:28080`는 오늘도 두 health endpoint 모두 즉시 실패했지만 container-internal app/DB/Redis probe는 4/4 성공했다.
  - 로컬 명령과 fresh test/build artifact가 `2026-07-27` future-dated timestamp를 남겨 오늘 기록의 시각 해석에 보정 정책이 계속 필요하다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - 운영 리스크 집계: 7건(Billing 회귀, build 실패, test XML write failure 재발, 브랜치 격차, host-path health 실패, local clock future timestamp, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `./gradlew test --tests com.faithlog.billing.presentation.BillingApiRestDocsTest --tests com.faithlog.billing.presentation.BillingControllerTest`
  - 이유: 오늘 재현된 실패 2건이 모두 Billing summary 응답/문서 계약에 집중돼 있다.
  - 기대 지표: Billing summary 계약 pass/fail, `monthlyByCategory` 필드 존재 여부, `monthlyPaidAmount` 계산값 일치 여부
  - `./gradlew test --rerun-tasks --warning-mode all --console=plain`
  - 이유: 같은 monitor run에서 XML test result write failure 11건이 다시 관찰돼 테스트 태스크 단독 재현성 확인이 필요하다.
  - 기대 지표: XML write failure 재현 여부, fresh test rerun 시 artifact 기록 안정성
  - `git switch develop && ./gradlew test --tests com.faithlog.poll.controller.PollApiRestDocsTest`
  - 이유: 최신 upstream `c7d67b4`가 poll 응답 DTO/REST Docs 계약을 수정했지만 dirty worktree와 branch-switch 미승인 상태라 checked-out branch에서 직접 검증하지 못했다.
  - 기대 지표: `allowUserOptionAdd` 노출 계약 pass/fail, 최신 upstream poll REST Docs 회귀 여부
  - `git switch develop && ./gradlew test jacocoTestReport`
  - 이유: local ref `origin/develop` snapshot 575 main / 102 test / 12 migrations를 현재 checked-out branch에서 직접 재검증하지 못했다.
  - 기대 지표: develop 기준 테스트 통과율, coverage 산출 여부, 최신 통합선 회귀 여부
- 오늘 트러블슈팅:
  - 미해결 관찰 1: Billing summary/doc contract 회귀.
  - 문제: Billing summary 관련 테스트 2건이 2026-07-26에도 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 응답 payload에서 `monthlyByCategory` 필드 4개가 누락됐고 `monthlyPaidAmount`가 기대값 `2500` 대신 `0`으로 반환됐다는 점뿐이다.
  - 조치 현황: 오늘 `./gradlew test`와 `./gradlew build`를 fresh rerun해 동일 실패를 다시 수집했다. 구현 변경은 하지 않았다.
  - 전후 지표: 2026-07-21 `138 tests / 2 failures`, 2026-07-26 `138 tests / 2 failures`
  - 재발 방지 후보: 승인된 변경 원천을 확인한 뒤 Billing summary DTO/집계/REST Docs 계약을 함께 재검증해야 한다.
  - 미해결 관찰 2: test XML result write failure 재발.
  - 문제: `./gradlew test`가 실패 테스트 2건 외에 `build/test-results/test/*.xml` 11건에 대해 XML 결과를 쓰지 못했다고 추가 출력했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 같은 실행 뒤 파일 개수 집계는 24 XML files를 유지했지만, 테스트 본문에는 XML write failure 11건이 보고됐다는 점뿐이다.
  - 조치 현황: 오늘은 fresh `test`와 `build` rerun으로 raw 오류 재현 여부를 분리 수집했다. `clean`이나 파일 삭제 같은 추가 조치는 하지 않았다.
  - 전후 지표: 2026-07-21 `build` run에서 XML write failure 미재현, 2026-07-26 `test` run에서 XML write failure 11건 재발
  - 재발 방지 후보: artifact 정리 정책, parallel test write 설정, 또는 file lock 조사 범위를 사용자 승인 없이 추정할 수 없으므로 다음 검증 범위를 먼저 확정해야 한다.
  - 미해결 관찰 3: host-path health probe 실패.
  - 문제: `127.0.0.1:28080`의 `/actuator/health`와 `/api/v1/health`가 오늘도 모두 즉시 연결 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 host-path probe 2건 모두 `http_code=000`이고, 같은 시점 container-internal app health / DB readiness / Redis ping은 성공했다는 점뿐이다.
  - 조치 현황: 오늘은 환경 변경 없이 host-path probe와 container-internal probe를 분리 기록했다.
  - 전후 지표: host-path `/actuator/health` 0/1 success, host-path `/api/v1/health` 0/1 success, container-internal app/DB/Redis probe 4/4 success
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 host-path failure 시 어떤 health source를 공식 metric으로 볼지 먼저 확정해야 한다.
  - 미해결 관찰 4: local clock future timestamp drift.
  - 문제: 이번 실행에서 `date`와 fresh test/build artifact가 monitor date보다 하루 앞선 `2026-07-27` timestamp를 기록했다.
  - 원인: 미확인. 현재는 로컬 clock 또는 세션 기준시각 불일치 가능성만 관찰됐다.
  - 조치 현황: 오늘은 보고 기준일을 `2026-07-26`으로 고정하고 raw timestamp를 그대로 기록했다.
  - 전후 지표: monitor date `2026-07-26`, local `date` 출력 `2026-07-27 06:01:39 +0900`, artifact timestamp `2026-07-27 06:03:26 +0900`
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 future-dated local clock 해석 정책을 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - maintenance candidate: 2026-07-26 기준 checked-out branch 품질 게이트는 138-test baseline 중 2개 Billing 회귀로 98.55% pass rate에 머물고 있고, 같은 monitor run에서 XML test result write failure 11건이 재발했다. 이는 개선 전후 비교가 가능한 troubleshooting backlog 근거로만 유지한다.

### 2026-07-21 Daily Monitor (checked-out branch)

- 브랜치/작업트리:
  - 기준 문서 확인: repo `AGENTS.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`를 확인하고 진행했다. 저장소에는 오늘도 `AGENT.md`가 없고 `AGENTS.md` 단일 규칙만 유지 중이다.
  - 보고 기준일은 화요일 `2026-07-21`이다. 로컬 `date` 출력은 `2026-07-22 06:02:38 +0900`으로 하루 앞서 있었으므로 future-dated local clock evidence로만 기록한다.
  - 현재 브랜치: `docs/37-poll-template-planning-sync`
  - `git status --short --branch`: 워크트리 변경 4건 (`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`), 미추적 빈 파일 1건 (`0`, 0 bytes, mtime `2026-06-30 22:58 +0900`)
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 213
- 변경 범위 수치:
  - 지난 자동화 실행 시각(`2026-07-20T21:01:35.468Z`) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 0건
  - 최신 upstream 실변경 커밋 참고: `c7d67b4 fix: #213 투표 조회에 항목 추가 허용값 노출 (#214)`, `2026-07-20 16:21:33 +0900`, 6 files changed / +28 / -2
  - 최신 upstream 주요 변경 파일: `docs/decision-log.md`, `src/docs/asciidoc/index.adoc`, `src/main/java/com/faithlog/poll/controller/dto/response/PollDetailResponse.java`, `src/main/java/com/faithlog/poll/controller/dto/response/PollListResponse.java`, `src/main/java/com/faithlog/poll/service/result/PollListItemResult.java`, `src/test/java/com/faithlog/poll/controller/PollApiRestDocsTest.java`
  - 현재 브랜치 고유 변경: `git diff --shortstat origin/develop...HEAD` 기준 3 files changed, 104 insertions(+), 모두 문서(`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`)
  - 현재 워크트리 미커밋 변경량: `git diff --shortstat` 기준 4 files changed, 2567 insertions(+), 9 deletions(-)
  - 같은 기간 코드/설정/의존성/마이그레이션 신규 upstream 관찰: 없음
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 main Java sources, 28 test Java sources, 55 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 8 top-level modules, 575 main Java sources, 102 test Java sources, DB 마이그레이션 12개
- 검증 신호:
  - `./gradlew test --warning-mode all --console=plain`: 실패, 1분 8초, `5 actionable tasks: 1 executed, 4 up-to-date`
  - `build/test-results/test/*.xml` 집계: 24 XML files, 138 tests / 2 failures / 0 errors / 0 skipped, 실행된 테스트 통과율 98.55%
  - 실패 테스트:
    - `BillingApiRestDocsTest.documents_charge_query_contracts`: REST Docs snippet mismatch, `data.monthlyByCategory[]`의 `paymentCategory`, `paidAmount`, `unpaidAmount`, `totalAmount` 필드 부재
    - `BillingControllerTest.charge_query_api_maps_my_summary_admin_campus_and_admin_member_responses`: `$.data.monthlyPaidAmount` expected `2500` but was `0`
  - deprecated 경고: configure 단계 `StartParameter.isConfigurationCacheRequested` 1건 지속
  - `./gradlew build --warning-mode all --console=plain`: 실패, 55초, `8 actionable tasks: 1 executed, 7 up-to-date`
  - 이번 실행에서는 2026-07-20에 보였던 `build/test-results/test/*.xml` XML write failure 11건이 다시 출력되지 않았다.
  - build artifact 시각: `build/reports/tests/test/index.html`, `build/reports/problems/problems-report.html` 모두 `2026-07-22 06:04:31 +0900`로 갱신됐다. monitor date `2026-07-21`보다 하루 앞선 future-dated local timestamp다.
- 운영/배포 신호:
  - `docker ps`: `faithlog-latest-app` Up 42 hours, `faithlog-latest-redis` / `faithlog-latest-postgres` Up 42 hours (`healthy`)
  - 별도 로컬 컨테이너 `timing-jeju-postgres`도 Up 20 hours 상태로 함께 관찰됐다. FaithLog 전용 상태 집계에는 포함하지 않았다.
  - 호스트 경로 `curl http://127.0.0.1:28080/actuator/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000207s`
  - 호스트 경로 `curl http://127.0.0.1:28080/api/v1/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000146s`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/actuator/health`: `{"status":"UP"}`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/api/v1/health`: `{"success":true,"code":"SUCCESS","message":"요청이 성공했습니다.","data":{"status":"UP"},"timestamp":"2026-07-21T21:03:22.767550755Z"}`
  - `docker exec faithlog-latest-postgres pg_isready -U faithlog -d faithlog`: `/var/run/postgresql:5432 - accepting connections`
  - `docker exec faithlog-latest-redis redis-cli ping`: `PONG`
- 오늘 리스크/관찰:
  - checked-out branch의 Billing summary/REST Docs 회귀 2건이 2026-07-21 monitor run에서도 그대로 재현됐다.
  - build 게이트는 오늘도 동일 2개 Billing 테스트 실패 때문에 막혀 있다.
  - 브랜치 격차가 `ahead 4 / behind 213`으로 유지돼 현재 브랜치 검증값을 최신 통합선 품질로 인용하기 어렵다.
  - 지난 자동화 실행 이후 신규 upstream 실변경 커밋은 없었다.
  - host-mapped app port `127.0.0.1:28080`는 두 health endpoint 모두 즉시 실패했지만 container-internal app/DB/Redis probe는 모두 성공했다.
  - 로컬 명령과 fresh test/build artifact가 `2026-07-22` future-dated timestamp를 남겨 오늘 기록의 시각 해석에 보정 정책이 계속 필요하다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - 전일 관찰된 build XML write failure 11건은 오늘 재현되지 않아, 현재는 지속 리스크보다 간헐 관찰치에 가깝다.
  - 운영 리스크 집계: 6건(Billing 회귀, build 실패, 브랜치 격차, host-path health 실패, local clock future timestamp, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `./gradlew test --tests com.faithlog.billing.presentation.BillingApiRestDocsTest --tests com.faithlog.billing.presentation.BillingControllerTest`
  - 이유: 오늘 재현된 실패 2건이 모두 Billing summary 응답/문서 계약에 집중돼 있다.
  - 기대 지표: Billing summary 계약 pass/fail, `monthlyByCategory` 필드 존재 여부, `monthlyPaidAmount` 계산값 일치 여부
  - `./gradlew build --rerun-tasks --warning-mode all --console=plain`
  - 이유: 2026-07-20에는 XML write failure 11건이 있었지만 2026-07-21에는 재현되지 않아 간헐성 여부 분리가 필요하다.
  - 기대 지표: XML write failure 재현 여부, build 재실행 시 artifact 기록 안정성
  - `git switch develop && ./gradlew test --tests com.faithlog.poll.controller.PollApiRestDocsTest`
  - 이유: 최신 upstream `c7d67b4`가 poll 응답 DTO/REST Docs 계약을 수정했지만 dirty worktree와 branch-switch 미승인 상태라 checked-out branch에서 직접 검증하지 못했다.
  - 기대 지표: `allowUserOptionAdd` 노출 계약 pass/fail, 최신 upstream poll REST Docs 회귀 여부
  - `git switch develop && ./gradlew test jacocoTestReport`
...
- 오늘 트러블슈팅:
  - 미해결 관찰 1: Billing summary/doc contract 회귀.
  - 문제: Billing summary 관련 테스트 2건이 2026-07-21에도 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 응답 payload에서 `monthlyByCategory` 필드 4개가 누락됐고 `monthlyPaidAmount`가 기대값 `2500` 대신 `0`으로 반환됐다는 점뿐이다.
  - 조치 현황: 오늘 `./gradlew test`와 `./gradlew build`를 fresh rerun해 동일 실패를 다시 수집했다. 구현 변경은 하지 않았다.
  - 전후 지표: 2026-07-20 `138 tests / 2 failures`, 2026-07-21 `138 tests / 2 failures`
  - 재발 방지 후보: 승인된 변경 원천을 확인한 뒤 Billing summary DTO/집계/REST Docs 계약을 함께 재검증해야 한다.
  - 미해결 관찰 2: build XML result write failure 증가.
  - 문제: `./gradlew build`가 실패 테스트 2건 이후 `build/test-results/test/*.xml` 18건에 대해 XML 결과를 쓰지 못했다고 추가 출력했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 `./gradlew test`에서는 24 XML files 집계가 가능했고, 직후 `./gradlew build`에서는 동일 테스트 실패와 함께 XML write 오류가 18건 보고됐다는 점뿐이다.
  - 조치 현황: 오늘은 fresh build rerun으로 raw 오류를 다시 수집했다. `clean`이나 파일 삭제 같은 추가 조치는 하지 않았다.
  - 전후 지표: 2026-07-20 build XML write failure 11건, 2026-07-21 build XML write failure 18건
  - 재발 방지 후보: artifact 정리 정책, parallel test write 설정, 또는 file lock 조사 범위를 사용자 승인 없이 추정할 수 없으므로 다음 검증 범위를 먼저 확정해야 한다.
  - 미해결 관찰 3: host-path health probe 실패와 container-internal permission gap.
  - 문제: `127.0.0.1:28080`의 `/actuator/health`와 `/api/v1/health`가 오늘도 모두 즉시 연결 실패했고, container-internal probe는 Docker socket permission denied 때문에 fresh 재시도에 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 host-path probe 2건 모두 `http_code=000`이고 `docker ps`는 성공했지만 `docker exec` 계열은 권한 오류로 막혔다는 점뿐이다.
  - 조치 현황: 오늘은 환경 변경 없이 host-path probe 실패와 permission gap을 분리 기록했다.
  - 전후 지표: host-path `/actuator/health` 0/1 success, host-path `/api/v1/health` 0/1 success, container-internal fresh probe 0/3 due to permission denial
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 escalated read-only Docker diagnostics 허용 여부와 health signal source 해석 정책을 먼저 확정해야 한다.
  - 미해결 관찰 4: local clock future timestamp drift.
  - 문제: 이번 실행에서 `date`와 Gradle 로그 본문이 monitor date보다 하루 앞선 `2026-07-22` timestamp를 기록했다.
  - 원인: 미확인. 현재는 로컬 clock 또는 세션 기준시각 불일치 가능성만 관찰됐다.
  - 조치 현황: 오늘은 보고 기준일을 `2026-07-21`로 고정하고 raw timestamp를 그대로 기록했다.
  - 전후 지표: monitor date `2026-07-21`, local `date` 출력 `2026-07-22 06:01:08 +0900`, Gradle 로그 timestamp `2026-07-22T06:02:41+09:00` / `2026-07-22T06:03:55+09:00`
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 future-dated local clock 해석 정책을 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - resume-ready maintenance candidate: 2026-07-21 기준 checked-out branch 품질 게이트는 138-test baseline 중 2개 Billing 회귀로 98.55% pass rate에 머물고 있고, build path에서는 XML result write failure 18건이 추가 관찰됐다. 이는 개선 전후 비교가 가능한 troubleshooting backlog 근거로만 유지한다.

### 2026-07-20 Daily Monitor (checked-out branch)
...
- 브랜치/작업트리:
  ...
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 213
- 변경 범위 수치:
  - 지난 자동화 실행 시각(`2026-07-19T21:09:18.263Z`) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 1건 (`c7d67b4 fix: #213 투표 조회에 항목 추가 허용값 노출 (#214)`, 2026-07-20 16:21:33 +0900)
  - 같은 기간 `origin/develop` 누적 변경량: 6 files changed, 28 insertions(+), 2 deletions(-)
  - 같은 기간 주요 변경 파일: `src/main/java/com/faithlog/poll/controller/dto/response/PollDetailResponse.java`, `src/main/java/com/faithlog/poll/controller/dto/response/PollListResponse.java`, `src/main/java/com/faithlog/poll/service/result/PollListItemResult.java`, `src/test/java/com/faithlog/poll/controller/PollApiRestDocsTest.java`, `src/docs/asciidoc/index.adoc`, `docs/decision-log.md`
  - 같은 기간 코드/설정/의존성 변경: upstream `#214`는 poll list/detail 응답과 REST Docs에 `allowUserOptionAdd` 노출을 추가한 소규모 API/문서 계약 정렬이다. 현재 checked-out branch에는 아직 반영되지 않았다.
  - 현재 브랜치 고유 변경: `git diff --shortstat origin/develop...HEAD` 기준 3 files changed, 104 insertions(+), 모두 문서(`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`)
  - 현재 워크트리 미커밋 변경량: `git diff --shortstat` 기준 4 files changed, 2479 insertions(+), 9 deletions(-)
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 main Java sources, 28 test Java sources, 55 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 10 top-level modules (`admin`, `batch`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `prayer`, `user`), 575 main Java sources, 102 test Java sources, DB 마이그레이션 12개
- 검증 신호:
  - `./gradlew test --warning-mode all --console=plain`: 실패, 1분 58초, `5 actionable tasks: 1 executed, 4 up-to-date`
  - `build/test-results/test/*.xml` 집계: 24 XML files, 138 tests / 2 failures / 0 errors / 0 skipped, 실행된 테스트 통과율 98.55%
  - 실패 테스트:
    - `BillingApiRestDocsTest.documents_charge_query_contracts`: REST Docs snippet mismatch, `data.monthlyByCategory[]`의 `paymentCategory`, `paidAmount`, `unpaidAmount`, `totalAmount` 필드 부재
    - `BillingControllerTest.charge_query_api_maps_my_summary_admin_campus_and_admin_member_responses`: `$.data.monthlyPaidAmount` expected `2500` but was `0`
  - deprecated 경고: configure 단계 `StartParameter.isConfigurationCacheRequested` 1건 지속
  - build artifact 시각: `build/reports/tests/test/index.html`, `build/reports/problems/problems-report.html` 모두 `2026-07-21 06:06:27 +0900`로 갱신됐다. monitor date `2026-07-20`보다 하루 앞선 future-dated local timestamp다.
  - 로컬 `date` 출력도 `2026-07-21 06:02:27 +0900`으로 monitor date `2026-07-20`보다 하루 앞섰다.
  - `./gradlew build --warning-mode all --console=plain`: 실패, 1분 16초, `8 actionable tasks: 1 executed, 7 up-to-date`
  - build 단계 추가 관찰: `./gradlew build`는 위 2개 Billing 테스트 실패 외에 `build/test-results/test/*.xml` 경로 11건에 대해 "Could not write XML test results" 오류를 추가 출력했다.
- 운영/배포 신호:
  - `docker ps`: `faithlog-latest-app` Up 18 hours, `faithlog-latest-redis` / `faithlog-latest-postgres` Up 18 hours (`healthy`)
  - 호스트 경로 `curl http://127.0.0.1:28080/actuator/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000277s`
  - 호스트 경로 `curl http://127.0.0.1:28080/api/v1/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000193s`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/actuator/health`: `{"status":"UP"}`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/api/v1/health`: `{"success":true,"code":"SUCCESS","message":"요청이 성공했습니다.","data":{"status":"UP"},"timestamp":"2026-07-20T21:03:32.382513044Z"}`
  - `docker exec faithlog-latest-postgres pg_isready -U faithlog -d faithlog`: `/var/run/postgresql:5432 - accepting connections`
  - `docker exec faithlog-latest-redis redis-cli ping`: `PONG`
- 오늘 리스크/관찰:
  - checked-out branch의 Billing summary/REST Docs 회귀 2건이 2026-07-20 monitor run에서도 그대로 재현됐다.
  - build 게이트는 오늘 fresh build rerun에서도 동일 2개 Billing 테스트 실패와 XML test result write failure 11건 때문에 계속 막혀 있다.
  - 브랜치 격차가 `ahead 4 / behind 213`까지 확대돼 현재 브랜치 검증값을 최신 통합선 품질로 인용하기 더 어려워졌다.
  - `origin/develop`에는 지난 실행 이후 poll 조회 응답의 `allowUserOptionAdd` 노출을 추가한 `#214` 1건이 들어왔지만, 현재 checked-out branch 검증 결과에는 아직 반영되지 않았다.
  - host-mapped app port `127.0.0.1:28080`는 두 health endpoint 모두 즉시 실패했지만 container-internal app/DB/Redis probe는 모두 성공했다.
  - 로컬 명령과 fresh test artifact가 `2026-07-21` future-dated timestamp를 남겨 오늘 기록의 시각 해석에 보정 정책이 계속 필요하다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - 운영 리스크 집계: 7건(Billing 회귀, build 실패, build XML write failure, 브랜치 격차, host-path health 실패, local clock future timestamp, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `./gradlew test --tests com.faithlog.billing.presentation.BillingApiRestDocsTest --tests com.faithlog.billing.presentation.BillingControllerTest`
  - 이유: 오늘 재현된 실패 2건이 모두 Billing summary 응답/문서 계약에 집중돼 있다.
  - 기대 지표: Billing summary 계약 pass/fail, `monthlyByCategory` 필드 존재 여부, `monthlyPaidAmount` 계산값 일치 여부
  - `./gradlew build --rerun-tasks --warning-mode all --console=plain`
  - 이유: 오늘 fresh build에서는 동일 2개 Billing 실패 외에 XML test result write failure 11건이 추가로 관찰돼 재현성 분리가 필요하다.
  - 기대 지표: XML write failure 재현 여부, 빌드 태스크 재실행 시 artifact 기록 안정성
  - `git switch develop && ./gradlew test --tests com.faithlog.poll.controller.PollApiRestDocsTest`
  - 이유: 최신 upstream `c7d67b4`가 poll 응답 DTO/REST Docs 계약을 수정했지만, dirty worktree와 branch-switch 미승인 상태라 checked-out branch에서 직접 검증하지 못했다.
  - 기대 지표: `allowUserOptionAdd` 노출 계약 pass/fail, 최신 upstream poll REST Docs 회귀 여부
  - `git switch develop && ./gradlew test jacocoTestReport`
  - 이유: 최신 upstream `c7d67b4`와 local ref `origin/develop` snapshot 575 main / 102 test / 12 migrations를 현재 checked-out branch에서 직접 재검증하지 못했다.
  - 기대 지표: develop 기준 테스트 통과율, coverage 산출 여부, 최신 통합선 회귀 여부
- 오늘 트러블슈팅:
  - 미해결 관찰 1: Billing summary/doc contract 회귀.
  - 문제: Billing summary 관련 테스트 2건이 2026-07-20에도 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 응답 payload에서 `monthlyByCategory` 필드 4개가 누락됐고 `monthlyPaidAmount`가 기대값 `2500` 대신 `0`으로 반환됐다는 점뿐이다.
  - 조치 현황: 오늘 `./gradlew test`를 fresh rerun해 동일 실패를 다시 수집했다. 구현 변경은 하지 않았다.
  - 전후 지표: 2026-07-19 `138 tests / 2 failures`, 2026-07-20 `138 tests / 2 failures`
  - 재발 방지 후보: 승인된 변경 원천을 확인한 뒤 Billing summary DTO/집계/REST Docs 계약을 함께 재검증해야 한다.
  - 미해결 관찰 2: build XML result write failure.
  - 문제: `./gradlew build`가 실패 테스트 2건 이후 `build/test-results/test/*.xml` 11건에 대해 XML 결과를 쓰지 못했다고 추가 출력했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 `./gradlew test`에서는 24 XML files 집계가 가능했고, 직후 `./gradlew build`에서는 동일 테스트 실패와 함께 XML write 오류가 추가로 보고됐다는 점뿐이다.
  - 조치 현황: 오늘은 fresh build rerun으로 raw 오류를 수집했고, `clean`이나 파일 삭제 같은 추가 조치는 하지 않았다.
  - 전후 지표: `./gradlew test` 집계 가능 XML 24 files, `./gradlew build` 추가 XML write failure 11건
  - 재발 방지 후보: build artifact 정리 정책이나 재현 절차를 사용자 승인 없이 추정할 수 없으므로, 다음 검증 범위를 먼저 확정해야 한다.
  - 미해결 관찰 3: host-path health probe 실패.
  - 문제: `127.0.0.1:28080`의 `/actuator/health`와 `/api/v1/health`가 오늘도 모두 즉시 연결 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 host-path probe 2건 모두 `http_code=000`이고, 같은 시점 container-internal app health / DB readiness / Redis ping은 성공했다는 점뿐이다.
  - 조치 현황: 오늘은 환경 변경 없이 host-path probe와 container-internal probe를 분리 기록했다.
  ...
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 host-path failure 시 어떤 health source를 공식 metric으로 볼지 먼저 확정해야 한다.
  - 미해결 관찰 4: local clock future timestamp drift.
  - 문제: 이번 실행에서 `date`와 fresh test artifact가 monitor date보다 하루 앞선 `2026-07-21` timestamp를 기록했다.
  - 원인: 미확인. 현재는 로컬 clock 또는 세션 기준시각 불일치 가능성만 관찰됐다.
  - 조치 현황: 오늘은 raw timestamp를 그대로 기록하고 2026-07-20 monitor run과 분리 해석했다.
  - 전후 지표: monitor date `2026-07-20`, local `date` 출력 `2026-07-21 06:02:27 +0900`, artifact timestamp `2026-07-21 06:06:27 +0900`
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 future-dated local clock 해석 정책을 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - upstream observed candidate: 지난 실행 이후 `origin/develop`에는 poll 조회 응답/문서 계약에 `allowUserOptionAdd`를 추가한 `#214`가 6 files / +28 / -2로 반영됐다. 단, 오늘 checked-out branch에서는 직접 재검증하지 못했으므로 구현 성과나 품질 수치로 승격하지 않는다.

### 2026-07-19 Daily Monitor (checked-out branch)

- 브랜치/작업트리:
  - 기준 문서 확인: Vault `AGENTS.md`, repo `AGENTS.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`를 확인하고 진행했다. 저장소에는 이번 실행에서도 `AGENT.md`가 없고 `AGENTS.md` 단일 규칙만 유지 중이다.
  - 현재 브랜치: `docs/37-poll-template-planning-sync`
  - `git status --short`: 워크트리 변경 4건 (`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`), 미추적 빈 파일 1건 (`0`, 0 bytes, mtime `2026-06-30 22:58 +0900`)
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 212
- 변경 범위 수치:
  - 지난 자동화 실행 시각(`2026-07-18T21:06:32.392Z`) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 1건 (`50b8a2d docs: #196 targeted after 측정 결과 기록 (#212)`, 2026-07-18 20:44:35 +0900)
  - 같은 기간 `origin/develop` 누적 변경량: 3 files changed, 33 insertions(+), 1 deletion(-)
  - 같은 기간 주요 변경 파일: `docs/decision-log.md`, `docs/resume-metrics.md`, `performance/k6/issue-196-prayer-poll-list-baseline/README.md`
  - 같은 기간 코드/설정/의존성 변경: 이번 실행에서 새로 관찰된 항목 없음. 가장 최근 upstream 구현 대량 변경은 여전히 2026-07-17 `5958e03 perf: #192-#199 성능 최적화 통합 (#209)`이다.
  - 현재 브랜치 고유 변경: `git diff --shortstat origin/develop...HEAD` 기준 3 files changed, 104 insertions(+), 모두 문서(`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`)
  - 현재 워크트리 미커밋 변경량: `git diff --shortstat` 기준 4 files changed, 2489 insertions(+), 9 deletions(-)
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 main Java sources, 28 test Java sources, 55 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 10 top-level modules (`admin`, `batch`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `prayer`, `user`), 575 main Java sources, 102 test Java sources, DB 마이그레이션 12개
- 검증 신호:
  - `./gradlew test --warning-mode all --console=plain`: 실패, 52초, `5 actionable tasks: 1 executed, 4 up-to-date`
  - `./gradlew build --warning-mode all --console=plain`: 실패, 40초, `8 actionable tasks: 1 executed, 7 up-to-date`
  - `build/test-results/test/*.xml` 집계: 24 XML files, 138 tests / 2 failures / 0 errors / 0 skipped, 실행된 테스트 통과율 98.55%
  - 실패 테스트:
    - `BillingApiRestDocsTest.documents_charge_query_contracts`: REST Docs snippet mismatch, `data.monthlyByCategory[]`의 `paymentCategory`, `paidAmount`, `unpaidAmount`, `totalAmount` 필드 부재
    - `BillingControllerTest.charge_query_api_maps_my_summary_admin_campus_and_admin_member_responses`: `$.data.monthlyPaidAmount` expected `2500` but was `0`
  - deprecated 경고: configure 단계 `StartParameter.isConfigurationCacheRequested` 1건 지속
  - build artifact 시각: `build/reports/tests/test/index.html` `2026-07-20 07:59:06 +0900`, `build/reports/problems/problems-report.html` `2026-07-20 07:59:07 +0900`로 갱신됐지만 monitor date(2026-07-19)보다 하루 앞선 future-dated timestamp가 찍혔다.
  - 로컬 `date` 출력도 `2026-07-20 08:01:28 +0900`으로 monitor date(2026-07-19)보다 하루 앞섰다.
- 운영/배포 신호:
  - `docker ps`: `faithlog-latest-app` Up 36 hours, `faithlog-latest-redis` / `faithlog-latest-postgres` Up 36 hours (`healthy`)
  - 호스트 경로 `curl http://127.0.0.1:28080/actuator/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000301s`
  - 호스트 경로 `curl http://127.0.0.1:28080/api/v1/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000110s`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/actuator/health`: `{"status":"UP"}`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/api/v1/health`: `{"success":true,"code":"SUCCESS","message":"요청이 성공했습니다.","data":{"status":"UP"},"timestamp":"2026-07-19T23:00:51.054690589Z"}`
  - `docker exec faithlog-latest-postgres pg_isready -U faithlog -d faithlog`: `/var/run/postgresql:5432 - accepting connections`
  - `docker exec faithlog-latest-redis redis-cli ping`: `PONG`
- 오늘 리스크/관찰:
  - checked-out branch의 Billing summary/REST Docs 회귀 2건이 2026-07-19 monitor run에서도 그대로 재현됐다.
  - build 게이트는 동일 2개 Billing 테스트 실패 때문에 계속 막혀 있다.
  - 브랜치 격차가 `ahead 4 / behind 212`까지 확대돼 현재 브랜치 검증값을 최신 통합선 품질로 인용하기 어렵다.
  - `origin/develop`에는 지난 실행 이후 문서/성능 측정 기록 커밋 `#212`만 추가됐고, 새로운 코드/설정/의존성 변경은 이번 실행 기준 추가 관찰되지 않았다.
  - host-mapped app port `127.0.0.1:28080`는 두 health endpoint 모두 즉시 실패했지만 container-internal app/DB/Redis probe는 모두 성공했다.
  - 로컬 명령과 fresh build artifact가 `2026-07-20` future-dated timestamp를 남겨 오늘 기록의 시각 해석에 보정 정책이 계속 필요하다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - 운영 리스크 집계: 6건(Billing 회귀, build 실패, 브랜치 격차, host-path health 실패, local clock future timestamp, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `./gradlew test --tests com.faithlog.billing.presentation.BillingApiRestDocsTest --tests com.faithlog.billing.presentation.BillingControllerTest`
  ...
  - 기대 지표: Billing summary 계약 pass/fail, `monthlyByCategory` 필드 존재 여부, `monthlyPaidAmount` 계산값 일치 여부
  - `git switch develop && ./gradlew test jacocoTestReport`
  - 이유: 최신 upstream `50b8a2d`와 local ref `origin/develop` snapshot 575 main / 102 test / 12 migrations를 현재 checked-out branch에서 직접 재검증하지 못했다.
  - 기대 지표: develop 기준 테스트 통과율, coverage 산출 여부, 최신 통합선 회귀 여부
- 오늘 트러블슈팅:
  - 미해결 관찰 1: Billing summary/doc contract 회귀.
  - 문제: Billing summary 관련 테스트 2건이 2026-07-19 monitor run에서도 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 응답 payload에서 `monthlyByCategory` 필드 4개가 누락됐고 `monthlyPaidAmount`가 기대값 `2500` 대신 `0`으로 반환됐다는 점뿐이다.
  - 조치 현황: 오늘 `./gradlew test`와 `./gradlew build`를 모두 fresh rerun해 동일 실패를 다시 수집했다. 구현 변경은 하지 않았다.
  - 전후 지표: 2026-07-17 `138 tests / 2 failures`, 2026-07-19 `138 tests / 2 failures`
  - 재발 방지 후보: 승인된 변경 원천을 확인한 뒤 Billing summary DTO/집계/REST Docs 계약을 함께 재검증해야 한다.
  - 미해결 관찰 2: host-path health probe 실패.
  - 문제: `127.0.0.1:28080`의 `/actuator/health`와 `/api/v1/health`가 모두 즉시 연결 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 host-path probe 2건 모두 `http_code=000`이고, 같은 시점 container-internal app health / DB readiness / Redis ping은 성공했다는 점뿐이다.
  - 조치 현황: 오늘은 환경 변경 없이 host-path probe와 container-internal probe를 분리 기록했다.
  - 전후 지표: host-path `/actuator/health` 0/1 success, host-path `/api/v1/health` 0/1 success, container-internal app/DB/Redis probe 4/4 success
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 host-path failure 시 어떤 health source를 공식 metric으로 볼지 먼저 확정해야 한다.
  - 미해결 관찰 3: local clock future timestamp drift.
  - 문제: 이번 실행에서 `date`와 fresh build artifact가 monitor date보다 하루 앞선 `2026-07-20` timestamp를 기록했다.
  - 원인: 미확인. 현재는 로컬 clock 또는 세션 기준시각 불일치 가능성만 관찰됐다.
  - 조치 현황: 오늘은 raw timestamp를 그대로 기록하고 2026-07-19 monitor run과 분리 해석했다.
  - 전후 지표: monitor date `2026-07-19`, local `date` 출력 `2026-07-20 08:01:28 +0900`, artifact timestamp `2026-07-20 07:59:06 +0900` / `2026-07-20 07:59:07 +0900`
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 future-dated local clock 해석 정책을 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - upstream observed candidate: 지난 실행 이후 `origin/develop`에는 `#212` 문서/측정 기록 커밋이 3 files / +33 / -1로 추가됐고 `performance/k6/issue-196-prayer-poll-list-baseline/README.md`가 새로 기록됐다. 구현/품질 새 수치로 승격할 근거는 이번 checked-out branch 검증만으로는 부족하다.

### 2026-07-17 Daily Monitor (checked-out branch)

- 브랜치/작업트리:
  - 기준 문서 확인: repo `AGENTS.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`, Obsidian `Projects/FaithLog/resume-metrics.md`, `Projects/FaithLog/decision-log.md`를 확인하고 진행했다. 저장소에는 오늘도 `AGENT.md`가 없고 `AGENTS.md` 단일 규칙만 유지 중이다.
  - 현재 브랜치: `docs/37-poll-template-planning-sync`
  ...
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 211
- 변경 범위 수치:
  - 현재 브랜치 고유 변경: `git diff --shortstat origin/develop...HEAD` 기준 3 files changed, 104 insertions(+), 모두 문서(`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`)
  - 지난 일일 기록(2026-07-16) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 1건 (`5958e03 perf: #192-#199 성능 최적화 통합 (#209)`, 2026-07-17 13:28:09 +0900)
  - 같은 기간 `origin/develop` 누적 변경량: 349 files changed, 57843 insertions(+), 109 deletions(-)
  - 같은 기간 주요 변경 축: `performance/k6/*` 대규모 성능 harness, `src/main/resources/db/migration/V12__add_performance_query_indexes.sql`, `src/main/java/com/faithlog/billing/infrastructure/repository/AdminChargeAggregationQueryRepository.java`, `src/test/java/com/faithlog/performance/*`, `src/test/java/com/faithlog/notification/service/NotificationRequestCommandServiceBatchTest.java`
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 main Java sources, 28 test Java sources, 55 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 10 top-level modules (`admin`, `batch`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `prayer`, `user`), 575 main Java sources, 102 test Java sources, DB 마이그레이션 12개
- 검증 신호:
  - `./gradlew test --warning-mode all --console=plain`: 실패, 17분 57초, `5 actionable tasks: 1 executed, 4 up-to-date`
  - `./gradlew build --warning-mode all --console=plain`: 실패, 36초, `8 actionable tasks: 1 executed, 7 up-to-date`
  - `build/test-results/test/*.xml` 집계: 24 XML files, 138 tests / 2 failures / 0 errors / 0 skipped, 실행된 테스트 통과율 98.55%
  - 실패 테스트:
    - `BillingApiRestDocsTest.documents_charge_query_contracts`: REST Docs snippet mismatch, `data.monthlyByCategory[]`의 `paymentCategory`, `paidAmount`, `unpaidAmount`, `totalAmount` 필드 부재
    - `BillingControllerTest.charge_query_api_maps_my_summary_admin_campus_and_admin_member_responses`: `$.data.monthlyPaidAmount` expected `2500` but was `0`
  - deprecated 경고: configure 단계 `StartParameter.isConfigurationCacheRequested` 1건 지속
  - build artifact 시각: `build/reports/tests/test/index.html`와 `build/reports/problems/problems-report.html`가 이번 실행에서 갱신됐지만 로컬 clock 기준 `2026-07-18` future-dated timestamp가 찍혔다.
- 운영/배포 신호:
  - 호스트 경로 `curl http://127.0.0.1:28080/actuator/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000154s`
  - 호스트 경로 `curl http://127.0.0.1:28080/api/v1/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000118s`
  - 오늘은 host-path probe만 fresh 수집했다. Docker/container-internal health 재확인은 이번 기록 시점까지 완료하지 못했다.
- 오늘 리스크/관찰:
  - checked-out branch의 Billing summary/REST Docs 회귀 2건이 2026-07-17에도 그대로 재현됐다.
  - build 게이트도 동일 2개 Billing 테스트 실패 때문에 막혀 있다.
  - 브랜치 격차가 `ahead 4 / behind 211`까지 확대돼 현재 브랜치 검증값을 최신 통합선 품질로 인용하기 어렵다.
  - `origin/develop`에는 2026-07-17 `#209` 성능 최적화 통합이 349 files / +57843 / -109 규모로 추가됐지만 dirty worktree 때문에 직접 재검증하지 못했다.
  - host-mapped app port `127.0.0.1:28080`는 두 health endpoint 모두 즉시 실패했다.
  - 로컬 명령과 fresh build artifact가 `2026-07-18` future-dated timestamp를 남겨 오늘 기록의 시각 해석에 별도 보정 메모가 필요하다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - 운영 리스크 집계: 7건(Billing 회귀 2건, build 실패, 브랜치 격차, upstream 미재검증, host-path health 실패 2건, local clock future timestamp, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `./gradlew test --tests com.faithlog.billing.presentation.BillingApiRestDocsTest --tests com.faithlog.billing.presentation.BillingControllerTest`
  - 이유: 오늘 재현된 실패 2건이 모두 Billing summary 응답/문서 계약에 집중돼 있다.
  - 기대 지표: Billing summary 계약 pass/fail, `monthlyByCategory` 필드 존재 여부, `monthlyPaidAmount` 계산값 일치 여부
  - `git switch develop && ./gradlew test jacocoTestReport`
  - 이유: 최신 upstream `5958e03`의 349-file 성능/배치/인덱스 변경은 현재 checked-out branch에서 직접 재검증되지 않았다.
  - 기대 지표: develop 기준 테스트 통과율, coverage 산출 여부, 최신 통합선 회귀 여부
  - `docker ps`, `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/actuator/health`, `docker exec faithlog-latest-postgres pg_isready -U faithlog -d faithlog`, `docker exec faithlog-latest-redis redis-cli ping`
  - 이유: 오늘은 host-path failure만 fresh 수집됐고 app 내부 health 및 dependency readiness는 다시 확인하지 못했다.
  - 기대 지표: 컨테이너 실행 여부, 내부 health success/fail, PostgreSQL readiness, Redis ping
- 오늘 트러블슈팅:
  - 미해결 관찰 1: Billing summary/doc contract 회귀.
  - 문제: Billing summary 관련 테스트 2건이 2026-07-17에도 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 응답 payload에서 `monthlyByCategory` 필드 4개가 누락됐고 `monthlyPaidAmount`가 기대값 `2500` 대신 `0`으로 반환됐다는 점뿐이다.
  - 조치 현황: 오늘은 `test`와 `build`를 모두 fresh rerun해 동일 실패를 재현하고 artifact를 갱신했다. 구현 변경은 하지 않았다.
  - 전후 지표: 2026-07-16 `138 tests / 2 failures`, 2026-07-17 `138 tests / 2 failures`
  - 재발 방지 후보: 승인된 변경 원천을 확인한 뒤 Billing summary DTO/집계/REST Docs 계약을 함께 재검증해야 한다.
  - 미해결 관찰 2: host-path health probe 실패.
  - 문제: `127.0.0.1:28080`의 `/actuator/health`와 `/api/v1/health`가 모두 즉시 연결 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 host-path probe 2건 모두 `http_code=000`이라는 점뿐이며, 이번 기록에서는 container-internal probe를 다시 수집하지 못했다.
  - 조치 현황: 오늘은 환경 변경 없이 host-path probe 실패만 fresh 기록했다.
  - 전후 지표: 2026-07-17 host-path `/actuator/health` 0/1 success, host-path `/api/v1/health` 0/1 success
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 host-path failure 시 read-only container diagnostics 허용 범위를 먼저 확정해야 한다.
  - 미해결 관찰 3: local clock future timestamp drift.
  - 문제: 이번 실행에서 `date`와 fresh build artifact가 monitor date보다 하루 앞선 `2026-07-18` timestamp를 기록했다.
  - 원인: 미확인. 현재는 로컬 clock 또는 세션 기준시각 불일치 가능성만 관찰됐다.
  - 조치 현황: 오늘은 raw timestamp를 그대로 기록하고 2026-07-17 일일 보고와 분리 해석했다.
  - 전후 지표: monitor date `2026-07-17`, local artifact timestamp `2026-07-18 06:33:38 +0900` / `2026-07-18 06:35:11 +0900`
  - 재발 방지 후보: `docs/decision-log.md`의 신규 pending question대로 clock drift 해석 정책을 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - upstream observed candidate: `origin/develop`에는 2026-07-17 `#209 성능 최적화 통합`이 추가됐고, 로컬 ref 기준 349 files / +57,843 / -109와 성능 harness, V12 인덱스 migration, 배치/집계 최적화 테스트가 함께 들어왔다. 단, 오늘 checked-out branch에서는 직접 재검증하지 못했다.

### 2026-07-16 Daily Monitor (checked-out branch)

- 브랜치/작업트리:
  - 기준 문서 확인: Vault `AGENTS.md`, repo `AGENTS.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`, Obsidian `Projects/FaithLog/resume-metrics.md`, `Projects/FaithLog/decision-log.md`를 확인하고 진행했다. 저장소에는 `AGENT.md`가 없고 `AGENTS.md` 단일 규칙만 유지 중이다.
  - 현재 브랜치: `docs/37-poll-template-planning-sync`
  - `git status --short --branch`: 워크트리 변경 4건 (`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`), 미추적 빈 파일 1건 (`0`, 0 bytes)
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 210
- 변경 범위 수치:
  - 지난 자동화 실행 시각(`2026-07-15T21:01:26.606Z`) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 1건 (`6796ed1 fix: #206 청구 페이징 동률 정렬 안정화 (#207)`, 2026-07-16 13:19:29 +0900)
  - 같은 기간 `origin/develop` 누적 변경량: 7 files changed, 131 insertions(+), 6 deletions(-)
  - 같은 기간 주요 변경 파일: `src/main/java/com/faithlog/billing/controller/BillingPageRequests.java`, `src/test/java/com/faithlog/billing/controller/BillingControllerTest.java`, `src/test/java/com/faithlog/billing/controller/BillingPageRequestsTest.java`, `src/test/java/com/faithlog/billing/controller/BillingApiRestDocsTest.java`, `src/docs/asciidoc/index.adoc`, `docs/decision-log.md`, `docs/resume-metrics.md`
  - 같은 기간 변경 영역 집계: `docs` 2 files, `src` 5 files, 신규 테스트 파일 1개 (`BillingPageRequestsTest.java`)
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 main Java sources, 28 test Java sources, 55 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 10 top-level modules (`admin`, `batch`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `prayer`, `user`), 569 main Java sources, 93 test Java sources, DB 마이그레이션 11개
- 검증 신호:
  - 이번 실행은 read-only 수집으로 제한했다. `./gradlew test`, `./gradlew build` fresh rerun은 수행하지 않았고 기존 artifact만 읽었다.
  - `build/test-results/test/*.xml` 집계: 24 XML files, 138 tests / 2 failures / 0 errors / 0 skipped, 실행된 테스트 통과율 98.55%
  - 실패 테스트:
    - `BillingApiRestDocsTest.documents_charge_query_contracts`: REST Docs snippet mismatch, `data.monthlyByCategory[]`의 `paymentCategory`, `paidAmount`, `unpaidAmount`, `totalAmount` 필드 부재
    - `BillingControllerTest.charge_query_api_maps_my_summary_admin_campus_and_admin_member_responses`: `$.data.monthlyPaidAmount` expected `2500` but was `0`
  - latest local test/build artifact 시각: `build/reports/problems/problems-report.html`, `build/reports/tests/test/index.html` 모두 `2026-07-16 06:06:44 +0900`
  - latest local snippet groups: 55
- 운영/배포 신호:
  - `docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'`: `faithlog-latest-app` Up 2 minutes, `faithlog-latest-redis` / `faithlog-latest-postgres` Up 37 hours (`healthy`), 보조 성능 측정용 postgres/redis 컨테이너 8개가 추가로 살아 있었다.
  - 호스트 경로 `curl http://127.0.0.1:28080/actuator/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000128s`
  - 호스트 경로 `curl http://127.0.0.1:28080/api/v1/health`: 즉시 연결 실패, `http_code=000`, `time_total=0.000403s`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/actuator/health`: `{"status":"UP"}`
  - 컨테이너 내부 `docker exec faithlog-latest-app wget -qO- http://127.0.0.1:8080/api/v1/health`: 3회 모두 `{"success":true,"code":"SUCCESS","message":"요청이 성공했습니다.","data":{"status":"UP"}}`
  - app startup log 관찰: `2026-07-16T21:00:21Z` 부팅, `2026-07-16T21:00:24Z` Flyway 11 migrations validate / schema version 11 확인, `2026-07-16T21:00:28Z` application started.
  - app startup warning 관찰: `2026-07-16T21:00:28Z` generated security password 경고 1건, `2026-07-16T21:00:28Z` SpringDoc `/api-docs` / `/swagger-ui.html` enabled 경고 2건
  - 추가 운영 한계: 같은 실행에서 `docker exec faithlog-latest-postgres ... pg_isready`, `docker exec faithlog-latest-redis redis-cli ping` 재확인은 docker API permission denied로 실패했다.
- 오늘 리스크/관찰:
  - checked-out branch의 최신 로컬 artifact는 여전히 Billing summary/REST Docs 회귀 2건을 유지한다.
  - 브랜치 격차가 `ahead 4 / behind 210`까지 확대돼 현재 브랜치 검증값을 최신 통합선 품질로 인용하기 어렵다.
  - `origin/develop`에는 지난 실행 이후 #206 billing paging tie-order 안정화 1건이 추가됐지만 dirty worktree 때문에 직접 재검증하지 못했다.
  - host-mapped app port는 즉시 실패하지만 container-internal health는 성공해 외부 가용성과 내부 런타임 신호가 분리돼 있다.
  - startup log에는 generated security password 경고와 SpringDoc endpoint enabled 경고가 남았지만, 이것을 local-only 관찰로 볼지 운영 리스크로 집계할지 사용자 결정이 아직 없다.
  - 이번 실행에서는 postgres/redis 내부 readiness 재확인까지는 못 갔다. 동일 세션의 docker API permission denied가 추가 증거 수집을 막았다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - 운영 리스크 집계: 7건(Billing 회귀 artifact, 브랜치 격차, upstream 미재검증, host-path health 실패, startup warning 해석 미정, docker API permission 제한, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `./gradlew test --tests com.faithlog.billing.presentation.BillingApiRestDocsTest --tests com.faithlog.billing.presentation.BillingControllerTest`
  - 이유: 최신 local artifact의 실패 2건이 모두 Billing summary 응답/문서 계약에 집중돼 있다.
  - 기대 지표: Billing summary 계약 pass/fail, `monthlyByCategory` 필드 존재 여부, `monthlyPaidAmount` 계산값 일치 여부
  - `git switch develop && ./gradlew test jacocoTestReport`
  - 이유: 최신 upstream `6796ed1`과 `origin/develop` snapshot 569 main / 93 test / 11 migrations를 현재 checked-out branch에서 직접 재검증하지 못했다.
  - 기대 지표: develop 기준 테스트 통과율, coverage 산출 여부, 최신 통합선 회귀 여부
  - `docker exec faithlog-latest-postgres pg_isready -U faithlog -d faithlog` 및 `docker exec faithlog-latest-redis redis-cli ping`
  - 이유: 이번 실행에서는 두 readiness probe가 docker API permission denied로 막혀 app-only health와 dependency readiness 사이에 증거 공백이 남았다.
  - 기대 지표: PostgreSQL readiness success/fail, Redis ping success/fail
- 오늘 트러블슈팅:
  - 미해결 관찰 1: Billing summary/doc contract 회귀.
  - 문제: Billing summary 관련 테스트 2건이 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 응답 payload에서 `monthlyByCategory` 필드 4개가 누락됐고 `monthlyPaidAmount`가 기대값 `2500` 대신 `0`으로 반환됐다는 점뿐이다.
  - 조치 현황: 이번 실행에서는 fresh rerun 없이 기존 XML/HTML artifact를 재집계했다. 구현 변경은 하지 않았다.
  - 전후 지표: 2026-07-15 `138 tests / 2 failures`, 2026-07-16 `138 tests / 2 failures`
  - 재발 방지 후보: 승인된 변경 원천을 확인한 뒤 Billing summary DTO/집계/REST Docs 계약을 함께 재검증해야 한다.
  - 미해결 관찰 2: host-path health probe 실패.
  - 문제: Docker는 app port `28080->8080`를 노출 중이지만 host `curl`은 즉시 연결 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 container-internal `/actuator/health` 성공, 내부 `/api/v1/health` 3/3 success, host-path TCP 연결 실패, postgres/redis readiness 재확인 permission 제한뿐이다.
  - 조치 현황: 오늘은 환경 변경 없이 host-path probe와 container-internal probe를 분리 기록했다.
  - 전후 지표: container-internal `/api/v1/health` 3/3 success, host-path `/actuator/health` 0/1 success, host-path `/api/v1/health` 0/1 success
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 어떤 경로를 운영 health source of truth로 볼지 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - upstream observed candidate: 지난 실행 이후 `origin/develop`에는 `#206` 청구 페이징 동률 정렬 안정화가 7 files / +131 / -6으로 반영됐고, Billing page request 정렬 계약과 회귀 테스트 3개가 함께 추가됐다. 단, 오늘 로컬 재검증은 미실행 상태다.

### 2026-07-15 Daily Monitor (checked-out branch)
...
- 브랜치/작업트리:
  ...
  - 현재 브랜치: `docs/37-poll-template-planning-sync`
  - `git status --short`: 워크트리 변경 4건 (`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`), 미추적 빈 파일 1건 (`0`, 0 bytes)
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 206
- 변경 범위 수치:
  ...
  - `git diff --shortstat`: 워크트리 미커밋 변경 4 files changed, 2164 insertions(+), 8 deletions(-)
  - 지난 자동화 실행 시각(`2026-07-13T21:01:02.061Z`) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 1건 (`bb29e01 feat: #188 #189 #190 경건 조회와 밥 정산 통합`)
  - 같은 기간 `origin/develop` 누적 변경량: 121 files changed, 6443 insertions(+), 92 deletions(-)
  - 같은 기간 주요 변경 파일: `src/main/resources/db/migration/V8__add_meal_poll_settlement.sql`, `src/main/java/com/faithlog/billing/controller/MealBillingController.java`, `src/main/java/com/faithlog/poll/controller/MealPollController.java`, `src/main/java/com/faithlog/devotion/service/AdminWeeklyDevotionQueryService.java`, `src/test/java/com/faithlog/billing/service/ChargeDevotionResubmissionIntegrationTest.java`, `src/test/java/com/faithlog/devotion/controller/AdminWeeklyDevotionControllerTest.java`
  - 변경 영역: `admin`, `billing`, `campus`, `devotion`, `global`, `poll`, `docs`, `build`
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 main Java sources, 28 test Java sources, 55 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 561 main Java sources, 85 test Java sources, DB 마이그레이션 8개
- 검증 신호:
  - `./gradlew test --warning-mode all --console=plain`: 실패, 1분 51초, 5 actionable tasks 중 1 executed / 4 up-to-date
  - `build/test-results/test/*.xml` 집계: 24 XML files, 138 tests / 2 failures / 0 errors / 0 skipped, 실행된 테스트 통과율 98.55%
  - 실패 테스트:
    - `BillingApiRestDocsTest.documents_charge_query_contracts`: REST Docs snippet mismatch, `data.monthlyByCategory[]`의 `paymentCategory`, `paidAmount`, `unpaidAmount`, `totalAmount` 필드 부재
    - `BillingControllerTest.charge_query_api_maps_my_summary_admin_campus_and_admin_member_responses`: `$.data.monthlyPaidAmount` expected `2500` but was `0`
  - `./gradlew build --warning-mode all --console=plain`: 실패, 40초, 동일 2개 Billing 테스트 실패 재현
  - deprecated 경고 상세: configure 단계 `StartParameter.isConfigurationCacheRequested` 경고 1건 지속
  - build problems report: `build/reports/problems/problems-report.html` 갱신 (`2026-07-15 06:04:34 +0900`, 129,870 bytes)
  - 테스트 HTML 리포트: `build/reports/tests/test/index.html` 갱신 (`2026-07-15 06:04:34 +0900`, 11,317 bytes)
- 운영/배포 신호:
  - `docker ps --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'`: `faithlog-latest-app`, `faithlog-latest-redis`, `faithlog-latest-postgres` 모두 Up 약 21시간, Redis/PostgreSQL은 Docker health `healthy`
  - 호스트 경로 `curl http://127.0.0.1:28080/actuator/health`: 즉시 연결 실패
  - 컨테이너 내부 `docker exec faithlog-latest-app ... /actuator/health`: `{"status":"UP"}`
  - 컨테이너 내부 `docker exec faithlog-latest-app ... /api/v1/health`: 3회 샘플 `29ms`, `6ms`, `8ms`
  - `docker exec faithlog-latest-postgres pg_isready -U faithlog -d faithlog`: accepting connections
  - `docker exec faithlog-latest-redis redis-cli ping`: `PONG`
- 오늘 리스크/관찰:
  - 현재 체크아웃 브랜치의 Billing summary 응답/REST Docs 회귀 2건이 오늘도 그대로 재현됐다.
  - 브랜치 격차가 `ahead 4 / behind 206`까지 확대돼 checked-out branch 검증값을 최신 통합선 품질로 인용할 수 없다.
  - `origin/develop`에는 #188/#189/#190 통합 커밋 1건으로 121 files / 6443 insertions / 92 deletions가 추가됐지만 현재 브랜치에서는 직접 재검증하지 못했다.
  - Gradle deprecated 경고 1건이 계속 관찰된다.
  - 컨테이너 내부 health는 성공했지만 host-mapped app port는 즉시 연결 실패라 외부 가용성으로 해석할 수 없다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - 운영 리스크 집계: 6건(체크아웃 브랜치 테스트 2건 실패, 브랜치 격차, upstream 미재검증, Gradle deprecation 경고, host-path health 실패, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `./gradlew test --tests com.faithlog.billing.presentation.BillingApiRestDocsTest --tests com.faithlog.billing.presentation.BillingControllerTest`
  - 이유: 오늘 실패 2건이 모두 Billing summary 응답/문서 계약에 집중돼 있다.
  - 기대 지표: Billing summary 계약 pass/fail, `monthlyByCategory` 필드 존재 여부, `monthlyPaidAmount` 계산값 일치 여부
  - `git switch develop && ./gradlew test && ./gradlew build --warning-mode all`
  - 이유: 최신 upstream `bb29e01`의 121-file 통합 변경은 현재 checked-out branch에서 직접 재검증되지 않았다.
  - 기대 지표: develop 기준 테스트 통과율, 빌드 성공 여부, 최신 통합선 회귀 여부
  - `curl http://127.0.0.1:28080/actuator/health` 또는 `curl http://127.0.0.1:28080/api/v1/health`
  - 이유: 현재는 container-internal health만 성공했고 host-path availability는 실패 상태다.
  - 기대 지표: host-path health 성공/실패, 외부 접근 기준 응답시간
- 오늘 트러블슈팅:
  - 미해결 관찰 1: Billing summary/doc contract 회귀.
  - 문제: Billing summary 관련 테스트 2건이 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 응답 payload에서 `monthlyByCategory` 필드 4개가 누락됐고 `monthlyPaidAmount`가 기대값 `2500` 대신 `0`으로 반환됐다는 점뿐이다.
  - 조치 현황: 오늘은 실패를 직렬 실행으로 재현하고 테스트 리포트/HTML artifact를 갱신했다. 구현 변경은 하지 않았다.
  - 전후 지표: 2026-07-14 checked-out branch `138 tests / 2 failures`, 2026-07-15 `138 tests / 2 failures`
  - 재발 방지 후보: 승인된 변경 원천을 확인한 뒤 Billing summary DTO/집계/REST Docs 계약을 함께 재검증해야 한다.
  - 미해결 관찰 2: host-path health probe 실패.
  - 문제: Docker는 app port `28080->8080`를 노출 중이지만 host `curl`은 즉시 연결 실패했다.
  - 원인: 미확인. 현재 관찰 가능한 사실은 container-internal health `UP`, PostgreSQL/Redis readiness 성공, host-path TCP 연결 실패뿐이다.
  - 조치 현황: 오늘은 환경 변경 없이 host-path probe와 container-internal probe를 분리 기록했다.
  - 전후 지표: container-internal `/api/v1/health` 3/3 success (`29ms`, `6ms`, `8ms`), host-path `/actuator/health` 0/1 success
  - 재발 방지 후보: `docs/decision-log.md`의 pending question대로 어떤 경로를 운영 health source of truth로 볼지 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - upstream observed candidate: `origin/develop`의 `bb29e01`은 #188/#189/#190 통합으로 121 files / 6443 insertions / 92 deletions와 V8 meal settlement migration을 포함한다. 커밋 메시지는 `449 tests`, Flyway V1-V8, Docker HTTP 45-step QA 통과를 주장하지만 현재 checked-out branch에서는 직접 재검증하지 못했다.
...
### 2026-07-13 Daily Monitor (checked-out branch)
...
- 브랜치/작업트리:
  ...
  - 현재 브랜치: `docs/37-poll-template-planning-sync`
  - `git fetch --all --prune`: 성공. `origin/audit/157-threat-model-authorization-matrix`, `origin/audit/158-jwt-session-lifecycle`, `origin/chore/156-user-auth-usecase-separation`, `origin/security/176-refresh-token-rotation-atomicity` 원격 ref가 정리됐다.
  ...
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 175
- 변경 범위 수치:
  - `git diff --name-only origin/develop...HEAD`: 현재 브랜치 고유 변경 파일 3개, 모두 문서 (`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`)
  - `git diff --shortstat origin/develop...HEAD`: 3 files changed, 104 insertions(+)
  - `git diff --shortstat`: 워크트리 미커밋 변경 4 files changed, 2018 insertions(+), 8 deletions(-)
  - 지난 자동화 실행 시각(`2026-07-11T21:00:49.769Z`) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 22건. 범위는 `#156` User/Auth 책임 분리, `#157` 위협 모델/인가 행렬, `#158` JWT session lifecycle audit, `#176` refresh rotation atomicity다.
  - 같은 기간 `origin/develop` 누적 변경량: 41 unique files changed, 2345 insertions(+), 307 deletions(-)
  - 변경 영역: `user`, `docs`
  - 의존성/CI 파일 변경: 관찰된 범위에서 없음
  - DB 마이그레이션 신규 변경: 현재 체크아웃 브랜치 0, 같은 기간 `origin/develop` 신규 변경 0
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 Java sources, 28 test Java sources, 1 test resource, 57 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 10 top-level modules, 513 Java sources, 75 test Java sources, DB 마이그레이션 6개
- 검증 신호:
  - `./gradlew test --warning-mode all --console=plain`: 성공, 22초, 5 actionable tasks up-to-date
  - `build/test-results/test/*.xml` 집계: 24 XML files, 138 tests / 0 failures / 0 errors / 0 skipped
  - `./gradlew build --warning-mode all --console=plain`: 성공, 1초, 8 actionable tasks up-to-date
  - deprecated 경고 상세: `StartParameter.isConfigurationCacheRequested` 경고 1건이 계속 출력됐다.
  - build problems report: `build/reports/problems/problems-report.html` 갱신 (`2026-07-13 06:04:15 +0900`, 129,871 bytes)
  - 테스트 HTML 리포트: `build/reports/tests/test/index.html`은 rerun이 `UP-TO-DATE`로 끝나 `2026-06-20 06:03:15 +0900`, 10,574 bytes 상태를 유지했다.
- 운영/배포 신호:
  - `docker ps --format '{{.Names}}'`: 성공, 실행 중 컨테이너 0개
  - health/latency 측정은 `docs/decision-log.md`의 `2026-06-17 - Daily Health And Response-Time Measurement Scope`, `2026-07-08 - Daily Monitor Local Runtime Startup Scope`, `2026-07-10 - Daily Monitor Upstream Revalidation Branch-Switch Scope`, `2026-07-11 - Daily Monitor Docker Recovery Scope` pending question이 남아 있어 오늘도 보류했다.
- 오늘 리스크/관찰:
  - 현재 체크아웃 브랜치 기준 신규 앱 코드 변경은 없고 로컬 검증 성공도 docs branch 기준이다.
  - 현재 브랜치와 `origin/develop`의 격차가 `ahead 4 / behind 175`까지 벌어져 checked-out branch 검증값을 최신 통합선 품질로 인용하면 과대해석 위험이 더 커졌다.
  - 최신 upstream에는 Java 소스 513개, 테스트 Java 소스 75개, 마이그레이션 6개가 있지만 현재 체크아웃 브랜치에는 각각 231개, 28개, 0개라 최신 통합선 품질을 직접 재검증하지 못했다.
  - Gradle deprecated 경고는 오늘도 동일하게 1건 재현됐다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - Docker daemon 접근 오류는 해소됐지만 실행 중 FaithLog 런타임이 없어 health/latency 측정은 여전히 비어 있다.
  - 운영 리스크 집계: 5건(브랜치 격차, upstream 미재검증, Gradle deprecated 경고 지속, 런타임 미기동으로 health 미측정, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `git switch develop && ./gradlew test && ./gradlew build --warning-mode all`
  - 이유: 오늘 성공값은 checked-out docs branch 기준이며, 최신 통합선 품질은 직접 재검증하지 못했다.
  - 기대 지표: develop 기준 테스트 통과율, 빌드 성공 여부, deprecated 경고 지속 여부
  - `git switch develop && ./gradlew test --tests '*Auth*' --tests '*Refresh*' --tests '*UserDeletion*'`
  - 이유: 지난 자동화 실행 이후 최신 upstream 실변경이 User/Auth 책임 분리와 refresh/session security 회귀에 집중돼 있기 때문이다.
  - 기대 지표: auth/refresh/security 회귀 pass/fail, 최신 테스트 수 변화
  - `git switch develop && ./gradlew test jacocoTestReport`
...
- 오늘 트러블슈팅:
  ...
  - 조치 현황: 오늘은 `./gradlew test --warning-mode all --console=plain`와 `./gradlew build --warning-mode all --console=plain`를 재실행해 경고 지속 여부만 재확인했다.
  - 전후 지표: 2026-07-12와 2026-07-13 모두 테스트/빌드 성공, deprecated 경고 1건 지속
  - 재발 방지 후보: Asciidoctor/Grolifant plugin 버전 검토 또는 대체 설정 확인
  - 미해결 관찰 2: 운영 측정 신호 미확보.
  - 문제: Docker CLI는 정상 응답했지만 실행 중 컨테이너가 없어 `/api/v1/health` 측정 대상을 확보하지 못했다.
  - 원인: 현재 세션에서 Docker daemon 자체는 접근 가능하지만 FaithLog local runtime은 기동되지 않은 상태로 관찰됐다.
  - 조치 현황: 오늘은 컨테이너 상태만 기록하고 Docker runtime 기동은 수행하지 않았다.
  - 전후 지표: 2026-07-12는 `docker ps`가 `EOF`로 실패했고, 2026-07-13은 `docker ps` 성공 / 실행 컨테이너 0개 / health 미측정
  - 재발 방지 후보: 로컬 런타임 자동 기동 허용 범위를 사용자 승인으로 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - upstream observed candidate: 지난 자동화 실행 이후 `origin/develop`에는 User/Auth 책임 분리와 security audit/refresh atomicity 중심 비-merge 커밋 22건이 누적됐고, 41개 파일과 2345 insertions가 관찰됐다. 단, 현재 브랜치 재검증 전까지는 로컬 검증 완료 성과로 승격하면 안 된다.

### 2026-07-12 Daily Monitor (checked-out branch)

- 브랜치/작업트리:
  - 기준 문서 확인: Vault `AGENTS.md`, repo `AGENTS.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`, Obsidian `Projects/FaithLog/resume-metrics.md`, `Projects/FaithLog/decision-log.md` 확인 후 진행. 저장소에는 `AGENT.md`가 없고 `AGENTS.md` 단일 규칙만 유지 중이다.
  - 현재 브랜치: `docs/37-poll-template-planning-sync`
  - `git fetch --all --prune`: 성공. `origin/chore/154-notification-fcm-usecase-separation`, `origin/chore/155-batch-scheduler-usecase-separation` 원격 ref가 정리됐다.
  - `git status --short --branch`: 워크트리 변경 4건 (`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`, `docs/resume-metrics.md`), 미추적 빈 파일 1건 (`0`)
  - 브랜치 divergence: `git rev-list --left-right --count origin/develop...HEAD` 기준 현재 브랜치 ahead 4, behind 152
- 변경 범위 수치:
  - `git diff --name-only origin/develop...HEAD`: 현재 브랜치 고유 변경 파일 3개, 모두 문서 (`docs/backend-implementation-policy.md`, `docs/codex/FAITHLOG_CODEX_HOOK.md`, `docs/decision-log.md`)
  - `git diff --shortstat origin/develop...HEAD`: 3 files changed, 104 insertions(+)
  - `git diff --shortstat`: 워크트리 미커밋 변경 4 files changed, 1901 insertions(+), 8 deletions(-)
  - 지난 자동화 실행 시각(`2026-07-10T21:02:16.184Z`) 이후 현재 체크아웃 브랜치 새 커밋: 0건
  - 같은 기간 `origin/develop` 비-merge 실변경 커밋: 18건. 범위는 `#153` Prayer 책임 분리, `#154` Notification/FCM 책임 분리, `#155` Batch/Scheduler 책임 분리 연속 커밋이다.
  - 같은 기간 `origin/develop` 누적 변경량: 43 unique files changed, 2676 insertions(+), 1324 deletions(-)
  - 변경 영역: `docs`, `prayer`, `notification`, `batch`
  - 의존성/CI 파일 변경: 관찰된 범위에서 없음
  - DB 마이그레이션 신규 변경: 현재 체크아웃 브랜치 0, 같은 기간 `origin/develop` 신규 변경 0
- 코드베이스 구조 수치:
  - 현재 체크아웃 브랜치: 8 top-level modules (`admin`, `billing`, `campus`, `devotion`, `global`, `notification`, `poll`, `user`), 231 Java sources, 28 test Java sources, 1 test resource, 57 snippet groups, 2 GitHub Actions workflows, DB 마이그레이션 0개
  - 로컬 `origin/develop` tree snapshot: 10 top-level modules, 498 Java sources, 73 test Java sources, DB 마이그레이션 6개
- 검증 신호:
  - `./gradlew test --warning-mode all --console=plain`: 성공, 9초, 5 actionable tasks up-to-date
  - `build/test-results/test/*.xml` 집계: 24 XML files, 138 tests / 0 failures / 0 errors / 0 skipped
  - `./gradlew build --warning-mode all --console=plain`: 성공, 9초, 8 actionable tasks up-to-date
  - deprecated 경고 상세: `StartParameter.isConfigurationCacheRequested` 경고 1건이 계속 출력됐다.
  - build problems report: `build/reports/problems/problems-report.html` 갱신 (`2026-07-12 06:02:58 +0900`, 129,871 bytes)
  - 테스트 HTML 리포트: `build/reports/tests/test/index.html`은 rerun이 `UP-TO-DATE`로 끝나 `2026-06-20 06:03:15 +0900`, 10,574 bytes 상태를 유지했다.
- 운영/배포 신호:
...
- 오늘 리스크/관찰:
  - 현재 체크아웃 브랜치 기준 신규 앱 코드 변경은 없고 로컬 검증 성공도 docs branch 기준이다.
  - 현재 브랜치와 `origin/develop`의 격차가 `ahead 4 / behind 152`까지 벌어져 checked-out branch 검증값을 최신 통합선 품질로 인용하면 과대해석 위험이 더 커졌다.
  - 최신 upstream에는 Java 소스 498개, 테스트 Java 소스 73개, 마이그레이션 6개가 있지만 현재 체크아웃 브랜치에는 각각 231개, 28개, 0개라 최신 통합선 품질을 직접 재검증하지 못했다.
  - Gradle deprecated 경고는 오늘도 동일하게 1건 재현됐다.
  - 루트의 미추적 빈 파일 `0`이 계속 남아 있다.
  - Docker 운영 신호는 오늘도 `EOF` 실패로 비어 있다.
  - 운영 리스크 집계: 5건(브랜치 격차, upstream 미재검증, Gradle deprecated 경고 지속, Docker 운영 신호 부재, untracked 파일 `0`)
- 오늘 테스트 후보:
  - `git switch develop && ./gradlew test && ./gradlew build --warning-mode all`
  - 이유: 오늘 성공값은 checked-out docs branch 기준이며, 최신 통합선 품질은 직접 재검증하지 못했다.
  - 기대 지표: develop 기준 테스트 통과율, 빌드 성공 여부, deprecated 경고 지속 여부
  - `git switch develop && ./gradlew test --tests com.faithlog.prayer.service.PrayerUseCaseServiceStructureTest --tests com.faithlog.notification.service.NotificationUseCaseServiceStructureTest --tests com.faithlog.batch.service.BatchSchedulerResponsibilityStructureTest`
  - 이유: 지난 자동화 실행 이후 최신 upstream 실변경이 `#153`~`#155` 책임 분리 구조 회귀에 집중돼 있기 때문이다.
  - 기대 지표: 구조 분리 회귀 테스트 pass/fail, package 경계 유지 여부
  - `git switch develop && ./gradlew test jacocoTestReport`
  - 이유: 최신 통합선 기준 coverage 산출 여부는 오늘도 직접 재확인하지 못했다.
  - 기대 지표: develop 기준 테스트 수, coverage HTML/XML 생성 성공 여부
- 오늘 트러블슈팅:
  ...
  - 원인: 기존 누적 기록 기준 Asciidoctor/Grolifant 계열 플러그인 경로가 deprecated API를 호출하는 것으로 계속 관찰된다.
  - 조치 현황: 오늘은 `./gradlew test --warning-mode all --console=plain`와 `./gradlew build --warning-mode all --console=plain`를 재실행해 경고 지속 여부만 재확인했다.
  - 전후 지표: 2026-07-11과 2026-07-12 모두 테스트/빌드 성공, deprecated 경고 1건 지속
  - 재발 방지 후보: Asciidoctor/Grolifant plugin 버전 검토 또는 대체 설정 확인
  - 미해결 관찰 2: 운영 측정 신호 미확보.
  ...
  - 전후 지표: 2026-07-11과 2026-07-12 모두 health 미측정, 2026-07-12는 컨테이너 수도 미확인
  - 재발 방지 후보: Docker 접근 복구나 로컬 런타임 기동 허용 범위를 사용자 승인으로 먼저 확정해야 한다.
- 오늘 이력서 bullet 후보:
  - checked-out branch 기준 신규 구현 성과 없음.
  - upstream observed candidate: 지난 자동화 실행 이후 `origin/develop`에는 Prayer, Notification/FCM, Batch/Scheduler 책임 분리 중심 비-merge 커밋 18건이 누적됐고, 43개 파일과 2676 insertions가 관찰됐다. 단, 현재 브랜치 재검증 전까지는 로컬 검증 완료 성과로 승격하면 안 된다.

### 2026-07-12

- #156 User와 Auth 유스케이스 책임 분리:
  - 기준: 최신 `origin/develop` `c5286cd`, 별도 worktree, `chore/156-user-auth-usecase-separation`.
  - TDD: 6개 전용 transaction, Controller 직접 연결, thin facade, 4개 support/FCM port와 cycle·SDK 누출 금지 구조 테스트를 production 수정 전에 추가해 6 tests / 5 failures RED를 확인하고 책임 이동 후 GREEN.
  - 분리: signup/login/refresh rotation/logout/users-me/withdrawal 6개 public Service, token 발급·ACTIVE campus 결과·session revocation·soft-delete 4개 package-private support, withdrawal 전체 FCM 비활성화 application port.
  - 줄 수: `AuthService` 188→55(-133, -70.7%), `UserAccountService` 103→19(-84, -81.6%). 추출 class를 포함한 전체 코드 감소가 아니라 compatibility facade 책임 축소 수치.
  - 검증: focused auth/user/role invalidation/FCM 성공, 전체 374 tests / 0 failures / 0 errors / 1 skipped, build/asciidoctor/diff-check 성공. API/DTO/ErrorCode/Entity/DB/Flyway/JWT security/Redis key·TTL/repository/dependency 변경 0건, Swagger annotation·Controller Entity 반환·SDK 누출·service cycle 0건.
  - 보안 후속: 기존 withdrawal의 마지막 active ADMIN 보호 부재는 운영 잠금 가능성이 있는 MEDIUM 항목으로 기록하고 동작 변경 없이 유지.
  - 제약: Project 카드 없음, 활성 pm-dev/.harness 없음. Data 볼륨 99%·가용 3.7GiB로 Docker QA를 prune/삭제 없이 생략했고 원격 Docker CI 필요. push/PR 미수행.
  - 이력서 후보: `Auth/User의 6개 공개 유스케이스를 전용 Service와 4개 응집 support로 분리해 188줄/103줄 통합 Service를 55줄/19줄 호환 facade로 70.7%/81.6% 축소하고, 6개 구조 게이트·374개 전체 테스트로 JWT·Redis rotation·FCM·soft-delete 계약 무변경을 검증했다.`

### 2026-07-11

- #155 Batch와 Scheduler 책임 분리:
  - 기준: 최신 `origin/develop` `66e8d7c`, 별도 worktree, `chore/155-batch-scheduler-usecase-separation`.
  - TDD: production 수정 전 구조 테스트 5건을 추가해 5/5 RED를 확인하고 책임 이동 후 GREEN. scheduler disabled Context와 coffee due close 재호출 시 정산 정확히 1회 characterization 보강.
  - 분리: scheduled Poll 생성, due coffee CLOSED/정산, 경건 미제출, 투표 미응답, 미납, 90일 stale FCM cleanup을 6개 전용 Service로 분리. Scheduler는 기존 8개 trigger에서 전용 job service만 호출하고 retention/PENDING recovery의 기존 TransactionTemplate 경계는 유지.
  - 줄 수: `PollAutomationService` 121→29(-92, -76.0%), `AutomaticNotificationService` 296→34(-262, -88.5%). 추출 class를 포함한 전체 코드 감소가 아니라 compatibility facade 책임 축소 수치.
  - 검증: 관련 6-domain 조합 283 tests / 0 failures / 0 errors / 0 skipped, 전체 368 tests / 0 failures / 0 errors / 1 skipped, build/asciidoctor/diff-check 성공. API/DTO/ErrorCode/Entity/DB/Flyway/repository/config/schedule/TTL/retry/retention/dependency 변경 0건, SDK 누출·순환 의존·Swagger annotation·Controller Entity 반환 0건.
  - 제약: GitHub `read:project` scope가 없어 카드 확인/이동 불가. `pm-dev/SKILL.md`, `.harness`, `harness.yaml` 부재로 FaithLog TDD gate 적용. 호스트 Data 볼륨 100%·가용 1.8GiB와 Docker socket 접근 거부로 삭제/prune 없이 Docker QA를 생략했고 원격 Docker CI가 필요하다. push/PR은 수행하지 않음.
  - 이력서 후보: `Batch/Scheduler의 Poll·자동 알림·FCM cleanup 책임을 6개 전용 use case로 분리하고 121줄/296줄 통합 Service를 29줄/34줄 호환 facade로 76.0%/88.5% 축소했으며, 5개 구조 게이트·368개 전체 테스트로 스케줄·정산·Redis fail-closed·retention 정책 무변경을 보장했다.`
